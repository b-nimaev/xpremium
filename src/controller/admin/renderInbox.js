    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 2:
                    _a.sent();
                    return [4 /*yield*/, client.db("xpremium").collection("proposals").find({ subscriber: false }).toArray().then(function (props) {
                            ctx.session.proposals = props;
                            var list = [];
                            // Перебираем массив заявок
                            props.forEach(function (element, indexOfElement) {
                                list.push(element.plan);
                            });
                            var newList = Array.from(new Set(list));
                            for (var index = 0; index < newList.length; index++) {
                                var plan = newList[index];
                                var key = [{ text: Messages_1.title[plan], callback_data: "plan " + plan }];
